...
export const Challenge = () => {
  ...
  {/* here we use useEffect to update the title tag when ever the "count" the count change it call the function of updating the title tag https://prnt.sc/vMLPQ7ZDzeaT because we pass 
    cuount as dependency arrey
    */}
  useEffect(() => {
    document.title = `Count: ${count}`;
  }, [count]);

  // here we use useEffect to console the name and if the name is empty https://prnt.sc/oFSOmcCyR8cU it should not run the function
  useEffect(() => {
    if (name !== '') {
      console.log(name);
    }
